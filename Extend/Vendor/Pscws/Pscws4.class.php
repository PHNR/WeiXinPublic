...
class PSCWS4 {
        ...
        while($tmpa = $this->get_result()) {
            foreach($tmpa as $tmp) {
                ...
                if(isset($list[$word])) {
                    ...
                    $list[$word] = array(
                        'word' => $tmp['word'], 'times' => 1, 'weight' => $tmp['idf'], 'attr' => $tmp['attr']
                    );
                }
            }
        }
    ...
    function _rule_load($fpath) {
        ...
        while($buf = fgets($fd, 512)) {
            ...
            $item = array(
                'tf'  => 5.0, 'idf' => 3.5, 'attr' => 'un', 'bit' => 0, 'flag' => 0, 'zmin' => 0, 'zmax' => 0,
                'inc' => 0, 'exc' => 0
            );
        ...
        if(count($this->_rd) > 0) {
            ...
            for($i = 0; $i < $zlen; $i++) {
                ...
                if(($r1['flag'] & PSCWS4_ZRULE_PREFIX) && ($i < ($zlen - $clen))) {
                    ...
                    $k            = $i + $clen;
                    $wmap[$i][$k] = array(
                        'tf'   => $r1['tf'], 'idf' => $r1['idf'], 'flag' => (PSCWS4_WORD_RULE | PSCWS4_WORD_FULL),
                        'attr' => $r1['attr']
                    );
                    $wmap[$i][$i]['flag'] |= PSCWS4_ZFLAG_WHEAD;
                ...
                if(($r1['flag'] & PSCWS4_ZRULE_SUFFIX) && ($i >= $clen)) {
                    ...
                    $wmap[$k][$i] = array(
                        'tf' => $r1['tf'], 'idf' => $r1['idf'], 'flag' => PSCWS4_WORD_FULL, 'attr' => $r1['attr']
                    );
