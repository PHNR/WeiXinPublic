...
class WxuserAction extends UserAction {
    ...
    protected function _initialize() {
        ...
        if($this->_get('token', 'trim')) {
            $wecha = M('Wxuser')->where(array(
                    'token' => $this->_get('token', 'trim'), 'uid' => session('uid')
                ))->find();
            if($wecha) {
            ...
            $wecha              = M('Wxuser')->where(array(
                    'token' => session('token'), 'uid' => session('uid')
                ))->find();
            $wecha['queryname'] = $this->getwxqueryname();
        }
    ...
    protected function getwxqueryname() {
        $tkinfo    = M('Token_open')->field('id,queryname')->where(array(
                'token' => session('wxtoken'), 'uid' => session('uid')
            ))->find();
