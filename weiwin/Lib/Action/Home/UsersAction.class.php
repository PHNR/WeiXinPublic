...
class UsersAction extends BaseAction {
    ...
    public function checklogin() {
        ...
        if($res && ($pwd === $res['password'])) {
            ...
            //
            $db->where(array('id' => $res['id']))->save(array(
                'lasttime' => $now, 'lastloginmonth' => $month, 'lastip' => $_SERVER['REMOTE_ADDR']
            )); //最后登录时间
    ...
    public function checkpwd() {
        ...
        $code = C('site_url') . U('Index/resetpwd', array(
                'uid' => $list['id'], 'code' => md5($list['id'] . $list['password'] . $list['email']),
                'resettime' => time()
            ));
