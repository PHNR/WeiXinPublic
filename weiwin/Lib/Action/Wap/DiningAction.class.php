...
class DiningAction extends BaseAction {
    ...
    public function cats() {
        ...
        if(isset($_GET['catid'])) {
            ...
            if(IS_POST) {
                ...
                $url = U('Dining/cats', array(
                        'token' => $this->token, 'wecha_id' => $this->wecha_id, 'keyword' => $key
                    ));
                $this->redirect($url);
            }
    ...
    public function dingdanpost() {
        ...
        if(IS_POST) {
            ...
            if($row['payment'] == 1) {
                ...
                // 结束
                $this->success('下单成功！', U('Dining/dingdan', array(
                            'token' => $this->token, 'wecha_id' => $this->wecha_id, 'success' => 1
                        )));
            }
    ...
    public function xiangqing() {
        $dining_cart_model = M('Product_cart');;
        $thisOrder = $dining_cart_model->where(array(
                'id' => intval($_GET['id']), 'wecha_id' => $this->wecha_id
            ))->find();
    ...
    public function deleteOrder() {
        ...
        $this->redirect(U('Dining/dingdan', array(
                    'token' => $_GET['token'], 'wecha_id' => $_GET['wecha_id'], 'dining' => 1
                )));
