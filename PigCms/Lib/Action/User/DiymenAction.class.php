...
class DiymenAction extends UserAction{
	...
	public function  class_del(){
		...
		if($class==false){
			...
			if($back==true){
				$this->success('删除成功',U('Diymen/index',array('token'=>session('token'))));
			}else{
	...
	public function  class_send(){
		if(IS_GET){
			...
			if($rt['rt']==false){
				...
				$this->success('操作成功',U('Diymen/index',array('token'=>session('token'))));
			}
			exit;
		}else{
			$this->error('非法操作',U('Diymen/index',array('token'=>session('token'))));
		}
	}
