...
class KtvAction extends UserAction{
	...
	public function photo_index(){
		...
		$count = $this->Ktv_photo->where($where)->count();

		$Page       = new Page($count,20);

		$show       = $Page->show();

		$data = $this->Ktv_photo->where($where)->limit($Page->firstRow.','.$Page->listRows)->order('show_order desc')->select();
	...
	public function photo_add(){
		...
		if(IS_POST){
			...
			if($this->Ktv_photo->add($_POST)){

				$this->success('添加成功！',U('Ktv/photo_index',array('token'=>$this->token)));
	...
	public function photo_set(){
		...
		$checkdata = $this->Ktv_photo->where(array('id'=>$id))->find();

		if(empty($checkdata)||$checkdata['token']!=$this->token){

            $this->error("没有相应记录.您现在可以添加.",U('Ktv/photo_add'));
		...
		if(IS_POST){ 
			...
			$check=$this->Ktv_photo->where($where)->find();

			if($check==false)$this->error('非法操作');

			if($this->Ktv_photo->create()){

				if($this->Ktv_photo->where($where)->save($_POST)){

					$this->success('修改成功',U('Ktv/photo_index',array('token'=>$this->token)));
	...
	public function photo_del(){
        ...
        if(IS_GET){                            
			...
			if($back==true){
				...
				$this->success('操作成功',U('Ktv/photo_index',array('token'=>$this->token)));

			}else{

				$this->error('服务器繁忙,请稍后再试',U('Ktv/photo_index',array('token'=>$this->token)));
	...
	//订单设置
	 public function setinfo(){ 
		...
		if(empty($list[0][name])){
			$list[0][name]="选择欢唱套餐";
			$list[0][value]="满200免包销|100包段欢乐唱|满300送酒水|买2小时送两小时";
		}
		//print_r($list);die;
		$this->assign('list',$list);
