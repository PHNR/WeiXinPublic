...
class WeiliveAction extends UserAction{
	...
	public function index(){
		...
		$count      = $data->where(array('token'=>$this->token))->count();
		$Page       = new Page($count,15);
		$show       = $Page->show();
		$list = $data->where(array('token'=>$this->token))->limit($Page->firstRow.','.$Page->listRows)->order('displayorder')->select();
	...
	public function category_add(){
		...
		if(IS_POST){
			...
			if($data['title']&&$data['displayorder']&&$data['summary']){
				...
				if($sql){
					$this->success('操作成功',U('Weilive/index',$where));
				}else{
					$this->error('服务繁忙，请稍候再试');
				}
			}else{
