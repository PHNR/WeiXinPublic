...
class ZhuangxiuAction extends UserAction
{
    ...
    {
        parent::_initialize();
        $function = M('Function')->where(array('funname' => 'zhuangxiu'))->find();
    ...
    public function son()
    {
        $where['token'] = session('token');
        $where['addtype'] = 'zhuangxiu';
        $this->assign('pid', M('Zhuangxiu')->where($where)->getField('id'));
        $estate_son = M('Zhuangxiu_son');
    ...
    public function son_del()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'addtype' => 'zhuangxiu');
        $check = $t_son->where($where)->find();
        if ($check == null) {
            $this->error('操作失败');
        } else {
    ...
    {
        $t_housetype = M('Zhuangxiu_housetype');
        $where['token'] = session('token');
        $where['addtype'] = 'zhuangxiu';
        $housetype = $t_housetype->where($where)->order('sort desc')->select();
        foreach ($housetype as $k => $v) {
            $son_type[] = M('Zhuangxiu_son')->where(array('id' => $v['son_id']))->field('id as sid,title')->find();
        }
    ...
    public function housetype_add()
    {
        ...
        $son_data = M('Zhuangxiu_son')->where(array('token' => session('token'),'addtype' => 'zhuangxiu'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
    ...
    {
        $housetype = M('Zhuangxiu_housetype');
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token);
    ...
    public function album_add()
    {
        ...
        if (IS_POST) {
            if ($check == NULL) {
                $check_ex = $t_album->where(array('token' => session('token'), 'poid' => $this->_post('poid'),'addtype'=>$this->_post('addtype')))->find();
                if ($check_ex) {
    ...
    public function reservation()
    {
        ...
        $t_company = M('Company');
        $token = session('token');
        $where =  array('token'=>$token,'isbranch'=>1,'shortname'=>'zhuangxiu');
        ...
        if(IS_POST){
            ...
            if($check == null){     
                    ...
                    if($t_company->add($_POST)){                     
                        $this->success('添加成功',U('Zhuangxiu/aboutus',array('token'=>session('token'))));
                        exit;
                    }else{
             ...
             if($t_company->where($wh)->save($_POST)){ 
                    $this->success('修改成功',U('Zhuangxiu/aboutus',array('token'=>session('token'))));
                    exit;
                }else{
