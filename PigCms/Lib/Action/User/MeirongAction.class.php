...
class MeirongAction extends UserAction
{
    ...
    {
        parent::_initialize();
        $function = M('Function')->where(array('funname' => 'meirong'))->find();
    ...
    public function son()
    {
        $where['token'] = session('token');
        $where['addtype'] = 'meirong';
        $this->assign('pid', M('Meirong')->where($where)->getField('id'));
        $estate_son = M('Meirong_son');
    ...
    public function son_del()
    {
        ...
        $id = (int) $this->_get('id');
        $token = $this->_get('token');
        $where = array('id' => $id, 'token' => $token,'addtype' => 'meirong');
        $check = $t_son->where($where)->find();
        if ($check == null) {
            $this->error('操作失败');
        } else {
    ...
    public function housetype()
    {
        ...
        foreach ($housetype as $k => $v) {
            $son_type[] = M('Meirong_son')->where(array('id' => $v['son_id']))->field('id as sid,title')->find();
        }
    ...
    public function housetype_add()
    {
        ...
        $son_data = M('Meirong_son')->where(array('token' => session('token'),'addtype' => 'meirong'))->field('id as sid,title')->select();
        $this->assign('son_data', $son_data);
