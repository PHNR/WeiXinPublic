...
if ($emailstatus == 1) {
    if ($content) {
        ...
        $mail->SetFrom($emailuser, '微享微信平台');
        $mail->AddReplyTo($email, '微享微信平台');
...
<div>请尽快登入微享网进行信息处理[<a href='http://www.zgweixiang.com/index.php?m=Index&a=login' target='_blank'>点击进入</a>]</div>
	...
	public function smss(){
		...
		if ($orders){
			...
			$str="您在微享平台有新订单（请查收）：订单类型：".$orderType."订单编号：".$thisOrder['id']."姓名：".$thisOrder['truename']."电话：".$thisOrder['tel']."下单时间：".date('Y-m-d H:i:s',$thisOrder['time'])."订购项目如下：";
			//
			$carts=unserialize($thisOrder['info']);
			//
			$totalFee=0;
