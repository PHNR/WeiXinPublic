...
class KtvAction extends BaseAction{
	...
	public function photo_index(){
		...
		$photo =M('ktv_photo')->where($where)->order('show_order desc')->select();
		//dump($data);exit;
	...
	public function photo_info(){
		...
		$photo = $this->Ktv_photo->where($where)->find();
		for($i=1;$i<6;$i++){
	...
	$info=M('Wxuser')->where(array('token'=>$this->token))->find();
        ...
        $ktvstatus=$info['ktvstatus'];
		$content = $this->sms();
		
		$phone=$info['phone'];
        $user=$info['smsuser'];//短信平台帐号
        $pass=md5($info['smspassword']);//短信平台密码
        $ktvsmsstatus=$info['ktvsmsstatus'];//短信平台状态
        $contentt = $this->smss();
        ...
        $mail->SetFrom('18267720632@163.com', '狗扑微信平台微信系统');
        $mail->AddReplyTo($email, '狗扑微信平台微信系统');
	...
	public function smss(){
		...
		$orders=$this->yuyue_order->where(array('token'=>$this->token,'type'=>$this->type))->order('date desc')->limit(0,1)->find();
			...
			$str="亲爱的狗扑微信平台网商户：以下是客户在你狗扑微信平台平台下的新KTV预约订单哦：订单类型：".$orders['kind']."姓名：".$orders['name']."电话：".$orders['phone']."预约日期：".$orders['or_date']."预约时间：".$orders['time']."下单时间：".$orders['date'];
			//
